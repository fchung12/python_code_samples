...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        batch_size = X.shape[0] # Need to resize bias to match batch size
        batch_b = np.repeat(self.params['b'],batch_size,axis=0)

        # Wx + b
        forward_output = np.matmul(X,self.params['W']) + batch_b
    ...
    def backward(self, X, grad):
        ...
        # Let o be the output of this layer:
        # o = Wx + b (forward output)
        # grad = dL/do
        
        # Calculations :
        # do/dW = x 
        # dL/dW = dL/do * do/dW = grad * X
        # Average gradient of mini batch
        batch_grad = np.zeros(self.gradient['W'].shape)
        batch_size = grad.shape[0]
        for b in range(0,batch_size):
            batch_grad += np.outer(X[b],grad[b]) # X * grad
        batch_grad = batch_grad/batch_size
        self.gradient['W'] = batch_grad
    
    
        # do/db = 1
        # dL/db = dL/do * do/db = 1 * grad
        batch_grad = np.zeros(self.gradient['b'].shape)
        for b in range(0,batch_size):
            batch_grad += grad[b] # 1 * grad
        batch_grad = batch_grad/batch_size
        self.gradient['b'] = batch_grad.flatten() # starter code in gradient checker requires this to be flattened to 1D
        
        # do/dX = W
        # dL/dx = dL/do * do/dX = grad * W
...
class relu:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        # o =  {X, if x > 0, 0 otherwise)(forward output)
        # grad = dL/do
        
        # do/dX = {1, if x > 0, 0 otherwise}
        # dL/dX = dL/do*do/dX = {grad, if x > 0, 0 otherwise}
        o = np.maximum(0,X)
        x_mask = np.where(o==0,0,1) # mask is 0 wherever relu is 0, 1 otherwise
        backward_output = np.multiply(x_mask,grad) #0 wherever relu is 0, 1*grad otherwise (same as calculation)
...
def gradient_checker(DataSet, model):
    ...
    for name, grad in grad_dict.items():
        ...
        ######################################################################################
        model[layer_name].params[param_name] -= epsilon # undo the add epsilon
        model[layer_name].params[param_name] -= epsilon
        _, _, _, f_w_sub_epsilon = forward_pass(model, x, y)
        model[layer_name].params[param_name] += epsilon # undo the sub epsilon to return to the original model values
